class OrderInfo {
  ...
  get pickupTime() {
    const pickupDate = new Date(this.order.fulfillments[0].pickupDetails.pickupAt).toLocaleDateString(
      "en-US",
      this.dateFormat
    );
    return pickupDate;
  ...
  get deliveryTime() {
    const deliveryTime = new Date(this.order.fulfillments[0].shipmentDetails.expectedShippedAt).toLocaleDateString(
      "en-US",
      this.dateFormat
    );
    return deliveryTime;
  ...
  get deliveryAddress() {
    return this.order.fulfillments[0].shipmentDetails.recipient.address.addressLine1;
  }
  // Returns delivery city, state, postal code as one line string
  get deliveryCityAndPostalCode() {
    const address = this.order.fulfillments[0].shipmentDetails.recipient.address;
  ...
  get card() {
    ...
    return this.order.tenders && this.order.tenders.length > 0 ? this.order.tenders[0].cardDetails.card : null;
  }
